 ...
 * application server, which constructs Validator object.
 * 
 * This object is injected into the tests so user can verify the validators are
 * working. This example does not touch validation on database layer, e.g. it is
 * not validating uniqueness constraint for email field.
 * 
 * 
...
public class MemberValidationTest {
	...
	public static Archive<?> createTestArchive() {
		return ShrinkWrap
				...
				.addClasses(BaseEntity.class, Member.class)
				// enable JPA
	...
	public void testRegisterEmptyMember() {
		...
		assertEquals("Four violations were found", 4, violations.size());
	}
	...
	public void testRegisterValidMember() {
		Set<ConstraintViolation<Member>> violations = validator
				.validate(createValidMember());

		assertEquals("No violations were found", 0, violations.size());
	}
	...
	public void testEmailViolation() {
		...
		assertEquals("One violation was found", 1, violations.size());
		assertEquals("Email was invalid",
				"n\u00E3o \u00E9 um endere\u00E7o de e-mail", violations
	...
	public void testPhoneViolation() {
		...
		assertEquals("One violation was found", 1, violations.size());
		assertEquals("Phone number was invalid",
				"tamanho deve estar entre 10 e 12", violations.iterator()
						.next().getMessage());
	}
